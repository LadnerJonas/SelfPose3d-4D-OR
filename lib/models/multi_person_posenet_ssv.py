        ...
        if epoch >= self.init_train_epochs_rootnet:
            ...
            if self.single_aug_training_posenet:
                ...
                if pred1[0].shape[0] > 0 and pred2[0].shape[0] > 0:
                    kps_2d_12 = [
                        cameras.project_pose_OR_4D_batch([p.to('cuda:0') for p in pred1], cam, trans2) for cam in proj_cameras
                    ]  # project the 3D poses to MV2
                    kps_2d_21 = [
                        cameras.project_pose_OR_4D_batch([p.to('cuda:0') for p in pred2], cam, trans1) for cam in proj_cameras
